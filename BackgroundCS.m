function [TotalCS BGCv BGCp TotalSulfMass Dpg] = BackgroundCS(Tag, Total, Mode)
...
logSigma = 0.7;  % choose a standard deviation for the distribution (see S&P fig 8.3 pg371 for typical values)
Sigma = exp(logSigma);
...
logDp = logDp_0:0.01:logDp_f;
 ...
 elseif (Tag==3)||(Tag==4)
  ...
  for i = 1:length(Dp)-1
    d(i) = Total/((2*pi)^(1/2)*logSigma)*exp(-(logDp(i)-logMode)^2/(2*(logSigma)^2)); %S&P pg 370, eq 8.54
...
RunningTotalV(1) = 0;
for i = 1:length(Dp)-1
    RunningTotalN(i+1) = RunningTotalN(i)+N(i);
    FractionN(i+1) = RunningTotalN(i+1)/TotalN;
    RunningTotalS(i+1) = RunningTotalS(i)+S(i);
    FractionS(i+1) = RunningTotalS(i+1)/TotalS;
    RunningTotalV(i+1) = RunningTotalV(i)+V(i);
    FractionV(i+1) = RunningTotalV(i+1)/TotalV;
end
IndexSigmaN = length(find(FractionN<0.84));
DpSigmaN = Dp(IndexSigmaN);
CalclogSigmaN = log(DpSigmaN/Dpg)
IndexSigmaS = length(find(FractionS<0.84));
DpSigmaS = Dp(IndexSigmaS);
CalclogSigmaS = log(DpSigmaS/DpgS)
IndexSigmaV = length(find(FractionV<0.84));
DpSigmaV = Dp(IndexSigmaV);
CalclogSigmaV = log(DpSigmaV/DpgV)
